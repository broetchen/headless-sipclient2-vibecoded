...
static void do_register(char *args)
{
    ...
    while ((t = strtok_r(NULL, " \t", &sv)) != NULL) {
        if      (!strncmp(t, "reg=",  4)) snprintf(g_reg_str,  sizeof(g_reg_str),  "%s", t + 4);
        else if (!strncmp(t, "user=", 5)) snprintf(g_user_str, sizeof(g_user_str), "%s", t + 5);
        else if (!strncmp(t, "pass=", 5)) snprintf(g_pass_str, sizeof(g_pass_str), "%s", t + 5);
    }
...
static void do_call(char *args)
{
    ...
    {
        char *sp = strpbrk(uri, " \t");
        if (sp) *sp = '\0';   /* nur die URI, Rest ignorieren */
    }
...
static void dispatch_line(char *line)
{
    ...
    if      (!strcasecmp(cmd, "register")) do_register(args);
    else if (!strcasecmp(cmd, "call"))     do_call(args);
    else if (!strcasecmp(cmd, "answer"))    do_answer(args);
    else if (!strcasecmp(cmd, "hangup"))    do_hangup(args);
    else if (!strcasecmp(cmd, "devs") ||
             !strcasecmp(cmd, "devices"))  do_devs();
    else if (!strcasecmp(cmd, "snd"))       do_snd(args);
    else if (!strcasecmp(cmd, "status"))    do_status();
    else if (!strcasecmp(cmd, "quit") ||
             !strcasecmp(cmd, "exit"))       do_quit();
    else
